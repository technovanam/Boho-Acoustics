...
const FloatingDbMeter = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_DB_METER_EVENT, onOpenRequest);
    window.addEventListener("keydown", onEscape);
    document.addEventListener("visibilitychange", onVisibilityChange);

    return () => {
      window.removeEventListener(OPEN_DB_METER_EVENT, onOpenRequest);
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-4 z-[60] flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-zinc-900 text-white shadow-[0_10px_24px_rgba(0,0,0,0.35)] transition-all duration-300 ease-out motion-safe:hover:-translate-y-1 motion-safe:hover:scale-105 motion-reduce:transition-none hover:border-primary/50 hover:bg-zinc-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:bottom-24 sm:right-6 sm:h-14 sm:w-14"
      >
        <Gauge className="h-5 w-5 sm:h-7 sm:w-7" />
      </button>

      {open && (
        <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/90 p-4 backdrop-blur-[20px] transition-all duration-500 sm:items-center sm:p-6 sm:bg-black/60 motion-reduce:transition-none motion-reduce:backdrop-blur-0" role="dialog" aria-modal="true" aria-labelledby="db-meter-title" aria-describedby="db-meter-description">
          <div className={`w-full max-w-md border border-white/10 bg-zinc-950 p-0 text-white shadow-[0_0_50px_rgba(0,0,0,1)] ${prefersReducedMotion ? "" : "animate-in fade-in-0 sm:zoom-in-95 duration-300"} motion-reduce:animate-none`}>
            {/* Header */}
            <div className="p-8 border-b border-white/5 flex items-start justify-between">
              <div>
                ...
                <h3 id="db-meter-title" className="font-display text-4xl font-bold tracking-tighter">dB Meter</h3>
              </div>
              <button
                type="button"
                onClick={handleClose}
                aria-label="Close dB meter"
                className="flex h-11 w-11 items-center justify-center rounded-full p-0 text-white/50 transition-colors motion-reduce:transition-none hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            {/* Main Meter Area */}
            <div className="p-8 space-y-12" id="db-meter-description">
              <div className="relative">
                ...
                <div className="relative h-4 w-full bg-white/5 overflow-hidden">
                  <div
                    className="absolute top-0 left-0 h-full bg-primary transition-[width] duration-200 ease-out motion-reduce:transition-none"
            ...
            <div className="grid grid-cols-2">
              <button
                type="button"
                onClick={handleClose}
                className="border-t border-r border-white/10 py-6 min-h-12 text-xs font-bold tracking-widest text-white/60 uppercase hover:bg-white/5 hover:text-white transition-all motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-inset"
              >
                Close
              </button>
              <button
                ...
                className="border-t border-white/10 py-6 min-h-12 text-xs font-bold tracking-widest text-primary-foreground bg-primary uppercase hover:opacity-90 disabled:opacity-50 transition-all motion-reduce:transition-none flex items-center justify-center gap-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-inset"
              >
                {isRunning ? <div className="w-2 h-2 rounded-full bg-white animate-pulse motion-reduce:animate-none" /> : <Mic className="h-4 w-4" />}
      ...
      {noisePopupOpen && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/85 p-4 backdrop-blur-[30px] animate-in fade-in duration-500 motion-reduce:animate-none motion-reduce:backdrop-blur-0" role="dialog" aria-modal="true" aria-labelledby="noise-alert-title" aria-describedby="noise-alert-description">
          <div className={`w-full max-w-lg border border-primary/30 bg-zinc-950 p-0 text-white shadow-[0_0_100px_rgba(218,157,64,0.15)] ${prefersReducedMotion ? "" : "animate-in fade-in duration-500"} motion-reduce:animate-none`}>
            <div className="p-10">
              <div className="flex items-start justify-between gap-6 mb-8">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <h4 id="noise-alert-title" className="font-display text-3xl font-bold tracking-tighter">Excessive Noise Detected</h4>
                  </div>
                </div>
                <button
                  type="button"
                  ...
                  className="flex h-11 w-11 items-center justify-center rounded-full p-0 text-white/50 hover:text-white transition-colors motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>

              <div className="space-y-6" id="noise-alert-description">
            ...
            <div className="grid grid-cols-2">
              <button
                type="button"
                onClick={dismissNoisePopup}
                className="border-t border-r border-white/10 py-6 min-h-12 text-[10px] font-bold tracking-[0.2em] text-white/60 uppercase hover:bg-white/5 hover:text-white transition-all motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-inset"
              >
                Dismiss Assessment
              </button>
              <Link
                to="/consultation"
                onClick={dismissNoisePopup}
                className="border-t border-white/10 py-6 min-h-12 text-[10px] font-bold tracking-[0.2em] text-primary-foreground bg-primary uppercase hover:opacity-90 transition-all motion-reduce:transition-none text-center flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-inset"
              >
