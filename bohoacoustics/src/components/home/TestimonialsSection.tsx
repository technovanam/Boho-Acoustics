...
const TestimonialsSection = () => (
  <section className="py-10 lg:py-20 bg-card border-t border-white/5 relative">
    <div className="container mx-auto px-6 lg:px-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 border-t border-l border-white/10">
        {testimonials.map((t) => (
          <div 
            key={t.name} 
            className="group relative p-8 lg:p-12 border-r border-b lg:border-b-0 border-white/10 transition-colors duration-500 bg-white/[0.01] lg:bg-transparent lg:hover:bg-white/[0.01]"
          >
            {/* Minimal Indicator (Sharp) */}
            <div className="absolute top-0 left-0 w-12 h-px bg-primary lg:bg-primary/20 lg:group-hover:bg-primary transition-colors" />
            
            <div className="flex flex-col h-full">
              <div className="flex gap-1 mb-8 opacity-100 lg:opacity-40 lg:group-hover:opacity-100 transition-opacity">
                {[...Array(5)].map((_, j) => (
                  <Star key={j} className="w-3 h-3 fill-primary text-primary" />
                ))}
              </div>

              <blockquote className="text-lg lg:text-xl font-display font-light leading-relaxed mb-12 flex-1 text-white lg:text-white/80 lg:group-hover:text-white transition-colors">
                "{t.text}"
              </blockquote>

              <div className="mt-auto flex flex-col space-y-4">
                <div className="h-[1px] w-16 bg-primary lg:w-8 lg:bg-primary/40 lg:group-hover:w-16 lg:group-hover:bg-primary transition-all duration-500" />
                <div>
                  <p className="font-display font-black text-lg tracking-tight text-primary lg:text-white lg:group-hover:text-primary transition-colors">
                    {t.name}
                  </p>
                  <p className="text-white/70 lg:text-white/45 text-[9px] tracking-[0.2em] font-bold uppercase transition-colors lg:group-hover:text-white/70">
                    {t.role}
                  </p>
                </div>
              </div>
